...
layui.define(['jquery','layer','element','form'], function(exports) {
	...
	var LI_NAV_CHILD = "nav-tree-ul-sid", LI_NAV_ITEM = "nav-tree-item", 
		...
		LI_DIV_SPREAD_OPEN = "icon-jian", LI_DIV_SPREAD_CLOSE = "icon-jia", LI_DIV_SPREAD_LAST = "icon-dian",
	...
	DTree.prototype.setting = function(options) {
		...
		/** 样式相关参数**/
		this.dot = (typeof (this.options.dot) === "boolean") ? this.options.dot : true;		//是否显示一级图标的小圆点
	...
	DTree.prototype.reloadSetting = function(options) {
		...
		/** 样式相关参数**/
		this.dot = (typeof (this.options.dot) === "boolean") ? this.options.dot : this.dot;		//是否显示一级图标的小圆点
	...
	DTree.prototype.getDom = function(treeId, parentId, title, isLast, iconClass, checkArr, level) {	
		...
		return {
			spread: function() {	// + - 图标
				return isLast ? "<i class='"+ICONFONT+" "+LI_DIV_SPREAD_LAST+" "+(!_this.dot ? ICON_HIDE : "")+" "+_this.style.ifont+"' data-spread='last' data-id='"+treeId+"' root-id='"+rootId+"'></i>" : 
					((level < _this.initLevel) ? "<i class='"+ICONFONT+" "+LI_DIV_SPREAD_OPEN+" "+_this.style.ifont+"' data-spread='open' data-id='"+treeId+"' root-id='"+rootId+"'></i>" : "<i class='"+ICONFONT+" "+LI_DIV_SPREAD_CLOSE+" "+_this.style.ifont+"' data-spread='close' data-id='"+treeId+"' root-id='"+rootId+"'></i>");
			},
			node: function() {	// 二级图标样式
				...
				if(nodeIcon != "-1" && leafIcon != "-1"){	// 都加载
					return isLast ? "<i class='"+ICONFONT+" "+leafIcons+" "+ICONFONTSPECIAL+" "+_this.style.ifont+" "+_this.style.icon+"' data-spread='last' data-id='"+treeId+"' root-id='"+rootId+"'></i>" : 
	...
	DTree.prototype.getLiItemDom =  function(dom, treeId, parentId, isLast, level, basicData, flag) {	
		...
		var div = "<div class='"+LI_DIV_ITEM+" "+_this.style.item+"' data-id='"+treeId+"' root-id='"+rootId+"' d-click='"+eventName.itemNodeClick+"' data-basic='"+basicData+"' d-contextmenu='false'>";
		if(_this.toolbar){
			if(_this.toolbarLoad == "node") {
				div = "<div class='"+LI_DIV_ITEM+" "+_this.style.item+"' data-id='"+treeId+"' root-id='"+rootId+"' d-click='"+eventName.itemNodeClick+"' data-basic='"+basicData+"' d-contextmenu='true'>";
			} 
	...
	DTree.prototype.dataInit = function(chooseId){	
		...
		$li_parents.children("."+LI_DIV_ITEM).children("i[data-spread]."+LI_DIV_SPREAD_CLOSE).addClass(LI_DIV_SPREAD_OPEN);
		$li_parents.children("."+LI_DIV_ITEM).children("i[data-spread]."+LI_DIV_SPREAD_CLOSE).removeClass(LI_DIV_SPREAD_CLOSE);
		$li_parents.children("."+LI_DIV_ITEM).children("i[data-spread]."+LI_DIV_NODE_CLOSE).addClass(LI_DIV_NODE_OPEN);
		$li_parents.children("."+LI_DIV_ITEM).children("i[data-spread]."+LI_DIV_NODE_CLOSE).removeClass(LI_DIV_NODE_CLOSE);
	...
	DTree.prototype.clickSpread = function($div) {	
		...
		if ($ul.length > 0) {
			if (spread == "close") {
				...
				$i_spread.removeClass(LI_DIV_SPREAD_CLOSE);
				$i_spread.addClass(LI_DIV_SPREAD_OPEN);
				...
				$i_spread.removeClass(LI_DIV_SPREAD_OPEN);
				$i_spread.addClass(LI_DIV_SPREAD_CLOSE);
	...
	DTree.prototype.changeTreeNodeDel = function(flag){
		...
		if(flag){
			...
			if($p_ul.children("li").length == 0){
				...
				$icon_i.eq(0).removeClass(LI_DIV_SPREAD_OPEN);
				$icon_i.eq(0).removeClass(LI_DIV_SPREAD_CLOSE);
				if(!_this.dot){$icon_i.eq(0).addClass(ICON_HIDE);}
				$icon_i.eq(0).addClass(LI_DIV_SPREAD_LAST);
				
				$icon_i.eq(1).removeClass(nodeIconArray[_this.nodeIcon]["open"]);
				$icon_i.eq(1).removeClass(nodeIconArray[_this.nodeIcon]["close"]);
		...
		if (_this.checkbarData == "change"){	//记录变更数据
			_this.obj.find("i[data-par]").each(function(){
				...
				if ($i.attr("data-checked") != $i.attr("data-initchecked")) {	
					...
					i_node.spread = $div.find("i[data-spread]").eq(0).attr("data-spread") == "open" ? true : false;
